...
MODEL_PATH =r"G:\Rabia-Salman\DomainBed\domainbed\results\GroupDRO_RESULTS_3\model.pkl"         # your GroupDRO checkpoint
...
plt.title("t-SNE Visualization by Domain (GroupDRO, Target = Sketch)", fontsize=14)
plt.legend(title="Domain", loc='best', frameon=True)
plt.tight_layout()
plt.savefig("tsne_by_domain.png", dpi=300)
plt.show()
...
plt.title("t-SNE Visualization by Class (GroupDRO, Target = Sketch)", fontsize=14)
plt.legend(title="Class", bbox_to_anchor=(1.05, 1), loc='upper left', frameon=True)
plt.tight_layout()
plt.savefig("tsne_by_class.png", dpi=300)
plt.show()
