...
class Config:
    def __init__(self):
        ...
        self.target_domain = 'photo'  # DANN trained on art_painting -> photo
        self.num_classes = 7
        self.checkpoint_dir = 'DANN_checkpoints'  # Directory for DANN checkpoints
        self.eval_output_dir = 'DANN_concept_shift_results'  # Directory for evaluation results
...
def evaluate_model(model, dataloader, device, return_features=False):
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            class_outputs, _, features = model(inputs)  # DANN returns 3 values
...
if __name__ == '__main__':
    ...
    # Load pretrained DANN checkpoint
    print("\n--- Loading DANN Model ---")
    checkpoint_path = os.path.join(cfg.checkpoint_dir, f"dann_{cfg.source_domain}_to_{cfg.target_domain}_final.pth")
    if not os.path.exists(checkpoint_path):
        # Try epoch 20 checkpoint if final doesn't exist
        checkpoint_path = os.path.join(cfg.checkpoint_dir, f"dann_{cfg.source_domain}_to_{cfg.target_domain}_epoch_20.pth")
    
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location=cfg.device)
        model_dann.load_state_dict(checkpoint['model_state_dict'])
        print(f"Successfully loaded DANN model from {checkpoint_path}")
        epoch = checkpoint.get('epoch', 'unknown')
        print(f"Model trained for {epoch} epochs")
    else:
        raise FileNotFoundError(f"No DANN checkpoint found at {checkpoint_path}. Available checkpoints in {cfg.checkpoint_dir}:")
    ...
    original_target_accuracy, original_target_true_labels, original_target_pred_labels = \
        evaluate_model(model_dann, DataLoader(full_target_test_dataset, batch_size=cfg.batch_size, shuffle=False, num_workers=4, pin_memory=True), cfg.device)
    print(f"Original Target Test Accuracy (DANN Model): {original_target_accuracy:.2f}%")
    ...
    label_shift_accuracy, label_shift_true_labels, label_shift_pred_labels = \
        evaluate_model(model_dann, shifted_label_loader, cfg.device)
    print(f"Target Test Accuracy with Label Shift (DANN): {label_shift_accuracy:.2f}%")
    ...
    rare_class_accuracy, rare_class_true_labels, rare_class_pred_labels = \
        evaluate_model(model_dann, rare_class_loader, cfg.device)
    print(f"Target Test Accuracy with Rare-Class Scenario (DANN): {rare_class_accuracy:.2f}%")
    ...
    plot_evaluation(original_target_true_labels, original_target_pred_labels,
                    f"({cfg.target_domain}) - DANN (Original Distribution)", "dann_original_target")

    print("\n--- Generating Visualizations for Label Shift Scenario ---")
    plot_evaluation(label_shift_true_labels, label_shift_pred_labels,
                    f"({cfg.target_domain}) - DANN (Label Shift)", "dann_label_shift_target")

    print("\n--- Generating Visualizations for Rare-Class Scenario ---")
    plot_evaluation(rare_class_true_labels, rare_class_pred_labels,
                    f"({cfg.target_domain}) - DANN (Rare Class)", "dann_rare_class_target")
    
    # Save summary metrics
    print("\n--- Saving Summary Metrics ---")
    summary_path = os.path.join(cfg.eval_output_dir, "dann_concept_shift_summary.txt")
    with open(summary_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("DANN MODEL - CONCEPT SHIFT EVALUATION\n")
        f.write("="*60 + "\n\n")
    ...
    print("\n" + "="*60)
    print("DANN CONCEPT SHIFT EVALUATION COMPLETE!")
    print("="*60)
